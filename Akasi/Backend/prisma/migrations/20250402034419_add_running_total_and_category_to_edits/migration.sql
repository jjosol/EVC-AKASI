/*
  ...
  - Added the required column `category_id` to the `EditsInverntory` table without a default value. This is not possible if the table is not empty.
  - Added the required column `runningTotal` to the `EditsInverntory` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `editsinverntory` ADD COLUMN `category_id` INTEGER NOT NULL,
    ADD COLUMN `runningTotal` INTEGER NOT NULL;

-- CreateIndex
CREATE INDEX `EditsInverntory_category_id_idx` ON `EditsInverntory`(`category_id`);
