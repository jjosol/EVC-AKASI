/*
  ...
  - The primary key for the `equipmentcategory` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `medCategory_id` on the `equipmentcategory` table. All the data in the column will be lost.
  - Added the required column `equipCategory_id` to the `equipmentCategory` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `equipmentcategory` DROP PRIMARY KEY,
    DROP COLUMN `medCategory_id`,
    ADD COLUMN `equipCategory_id` INTEGER NOT NULL AUTO_INCREMENT,
    ADD PRIMARY KEY (`equipCategory_id`);
