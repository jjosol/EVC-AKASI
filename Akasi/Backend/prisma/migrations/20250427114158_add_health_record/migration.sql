...
-- AlterTable
ALTER TABLE `consultation_records` ADD COLUMN `health_record_created` BOOLEAN NOT NULL DEFAULT false;
...
CREATE TABLE `HealthRecord` (
    ...
    UNIQUE INDEX `HealthRecord_consultation_id_key`(`consultation_id`),
    PRIMARY KEY (`id`)
...
-- AddForeignKey
ALTER TABLE `HealthRecord` ADD CONSTRAINT `HealthRecord_consultation_id_fkey` FOREIGN KEY (`consultation_id`) REFERENCES `consultation_records`(`consultation_id`) ON DELETE RESTRICT ON UPDATE CASCADE;
