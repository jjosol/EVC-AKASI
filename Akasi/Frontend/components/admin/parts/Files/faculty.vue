<template>
  ...
  <div class="faculty-container">
    ...
    <ClientOnly>
      ...
      <div v-else-if="faculty.length === 0" class="text-center py-8 text-gray-500">
        No faculty found
      </div>
  ...
  >
    <div class="bg-white rounded-lg shadow-xl w-full max-w-5xl mx-4 my-8 overflow-hidden">
      ...
      <div class="bg-[#2f4a71] text-white p-5 flex justify-between items-center">
        <h3 class="text-xl font-bold">Faculty Information</h3>
      ...
      <div class="p-6 border-b border-gray-200" v-if="selectedFaculty">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p class="text-lg">{{ selectedFaculty.section || 'N/A' }}</p>
          </div>
        </div>
      </div>
...
const fetchFaculty = async () => {
  ...
  try {
    const possiblePaths = [
      ...
      '/api/api/get-clients/faculty',
      'http://localhost:3001/api/get-clients/faculty',
      '/api/faculty',
      'http://localhost:3001/faculty',
      '/api/clients/faculty',
      'http://localhost:3001/clients/faculty'
    ];
...
// Modal functions
const openFacultyModal = (member) => {
  selectedFaculty.value = member;
...
const fetchFacultyFiles = async () => {
  ...
  facultyFiles.value = [];
  const staffId = selectedFaculty.value.client_id;
  ...
  try {
    ...
    if (result && result.success) {
      facultyFiles.value = result.data;
      console.log(`Successfully fetched ${facultyFiles.value.length} files for faculty ${staffId}`);
    } else {
      throw new Error('Invalid response format');
    }
  } catch (err) {
    console.error('Error fetching faculty files:', err);
