...
import { useProfile } from '~/composables/useProfile2';
...
const validateForm = () => {
  ...
  // Use optional chaining and check for either client_id or id property
  const clientId = profile.value?.client_id || profile.value?.id;
  
  if (!clientId) {
    statusMessage.value = 'Unable to determine client ID. Please refresh.';
    statusType.value = 'error';
    return false;
  }
...
const submitAppointment = async () => {
  ...
  try {
    ...
    if (loading.value) {
      statusMessage.value = 'Profile is still loading, please wait...';
      statusType.value = 'error';
      return;
    }
    
    if (error.value) {
      statusMessage.value = 'Error loading profile: ' + error.value;
      statusType.value = 'error';
      return;
    }
