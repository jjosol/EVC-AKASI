...
// Modified computed property to detect name-edit-only mode
const modalTitle = computed(() => {
  if (props.editData?.nameEditOnly) {
    return 'Edit Equipment Name';
  }
...
watch(() => props.editData, (data) => {
  if (data) {
    equipment.value = {
      ...
      nameEditOnly: data.nameEditOnly || false
    };
  } else {
    resetForm();
  }
...
const validateForm = () => {
  ...
  // Only validate these fields if not in name-edit-only mode
  if (!equipment.value.nameEditOnly) {
...
const prepareSubmit = () => {
  ...
  if (equipment.value.nameEditOnly) {
    confirmationMessage.value = `You are about to change the equipment name to "${equipment.value.name}". This will be logged.`;
  } else {
...
const submitForm = async () => {
  try {
    ...
    if (isEditMode.value) {
      // Handle name-edit-only mode separately
      if (equipment.value.nameEditOnly) {
        result = await inventoryService.updateEquipmentNameOnly(
          props.editData.equip_id,
          equipment.value.name
        );
      } else {
        // Update all equipment properties
...
const fetchCategories = async () => {
  try {
    ...
    // No longer create a default category if none exist
  } catch (error) {
...
<template>  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="z-10 w-full max-w-md mx-4 p-4 sm:p-6 bg-white rounded-lg shadow-lg">
      ...
      <form @submit.prevent="prepareSubmit">
        <!-- Equipment Name field (always shown) -->
        <div class="mb-4">
          ...
          <input 
            ...
            :disabled="isEditMode && !equipment.nameEditOnly"
            :class="{'bg-gray-100': isEditMode && !equipment.nameEditOnly}"
            placeholder="Enter equipment name"
            required
          />
          <p v-if="formErrors.name" class="mt-1 text-sm text-red-500">{{ formErrors.name }}</p>
          <p v-if="isEditMode && !equipment.nameEditOnly" class="mt-1 text-xs text-gray-500">
            Equipment name cannot be changed. Create a new item if needed.
          </p>
        </div>
        
        <!-- Only show these fields if NOT in name-edit-only mode -->
        <template v-if="!equipment.nameEditOnly">
          <!-- Category Selection -->
          <div class="mb-4">
        ...
        <div class="flex justify-end space-x-2">
          ...
          >
            {{ equipment.nameEditOnly ? 'Update Name' : (isEditMode ? 'Update Equipment' : 'Add Equipment') }}
          </button>
        </div>
      </form>
    </div>
  ...
  <ConfirmationModal
    ...
    :confirmButtonText="equipment.nameEditOnly ? 'Update Name' : (isEditMode ? 'Update' : 'Add')"
