...
const basic = require('./shared/consola.v8r3PyFm.cjs');
const p$1 = require('node:process');
...
const p__default = /*#__PURE__*/_interopDefaultCompat(p$1);
...
function isUnicodeSupported() {
	const {env} = p__default;
	const {TERM, TERM_PROGRAM} = env;

	if (p__default.platform !== 'win32') {
...
class FancyReporter extends basic.BasicReporter {
  formatStack(stack, opts) {
    ...
    return `
${indent}` + basic.parseStack(stack).map(
...
function createConsola(options = {}) {
  ...
  const consola2 = core.createConsola({
    ...
    reporters: options.reporters || [
      options.fancy ?? !(T || A) ? new FancyReporter() : new basic.BasicReporter()
    ],
    ...options
  });
  return consola2;
